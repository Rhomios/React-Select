...
const DropdownSelect: FC<Props> = ({
  ...
  const handleSearch = (searchValue: string) => {
    if (searchValue.length !== 0) {
      ...
      searchRecords = options.filter(
        (i) =>
          i.value
            .toString()
            .toLowerCase()
            .search(new RegExp(searchValue.toLowerCase().trim())) !== -1,
      );
      ...
      searchRecords.sort((a: Option, b: Option) => {
        if (a.value.toLowerCase().startsWith(searchValue.toLowerCase())) {
          ...
          !b.value.toLowerCase().startsWith(searchValue.toLowerCase())
        ) {
          return 1;
        } else {
          return 0;
  ...
  return (
    ...
    >
      <div
        className={`dropdown-button ${size} ${variant} `}
      ...
      >
            ...
            <>
              {selectedItems.map((i) => (
                <div key={i.value} className={`selected-item ${variant}`}>
                  <div className={`selected-item-value`}>{i.value}</div>
                  <div
                    className={`selected-item-remove-btn`}
        ...
        <div className="selected-items-options">
          {selectedItems.length !== 0 && (
            <div
              className={`${"selected-items-clear-btn"} ${variant}`}
      ...
      <div
        className={`dropdown-menu-container ${
          !isOpen && "dropdown-menu-container-on-hide"
        }`}
      >
        <ul className="dropdown-menu-options">
          ...
          {optionList.length !== 0 ? (
              ...
              {optionList.map((i) => (
                <li
                  ...
                  className={`dropdown-option-item ${
                    isInArray(selectedItems, i) && "dropdown-button-active"
                  }`}
                >
                  {i.icon && (
                    <div className={"dropdown-option-item-icon-container"}>
                      <img
                        className={"dropdown-option-item-icon"}
                        src={i.icon}
                      ></img>
                    </div>
                  )}
            ...
            <div className={"no-options-warning"}>No options found</div>
          )}
        </ul>
      </div>
    </div>
